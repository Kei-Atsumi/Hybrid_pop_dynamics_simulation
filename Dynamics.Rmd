...
Directory: `/mnt/c/Users/katsu/Dropbox/5_Simulation`
...
make_config <- function(
  ...
  paste(
    ...
    # "DumpNatSelProb\t=\tOff", 
    "GeneFile\t=\tgenes.txt",
    paste0("PhenotypeFile\t=\tphenotypes_", gset, ".txt"),
    paste0("OutputFolder\t=\t/Users/katsu/Dropbox/5_Simulation/Result/", gset, "_", sexsel, "_natsel", natsel, "/0"), #Output folder
    paste0("NaturalSelection\t=\t../Config_shared/naturalsel_", natsel, ".txt"),                     #Natural selection file
    paste0("SexualSelection\t=\tsexualsel_", sexsel, ".txt"), #Sexual selection file
    #population names:
    "pop2_name\t=\tp2",
    ...
    "samplegens\t=\t../Config_shared/samplegens.txt", 
...
# Create gene file
gf <- make_genome()
write_tsv(gf, "./Config_discrete/genes.txt")
...
write_tsv(Pheno, "./Config_discrete/phenotypes_dis_unlink.txt")

# dis_link
Pheno[5,2] <- gf[2+g_chrom*2, "name"]
write_tsv(Pheno, "./Config_discrete/phenotypes_dis_link.txt")
  ...
  for(ss in c("speriorhyb", "disruptive", "flip", "heterorandom", "pdominance", "recessive", "selfref")){
    for(ns in c(0.3, 1)){
      make_config(gset = genomeset, sexsel = ss, natsel = ns) %>% 
        write(paste0("./Config_discrete/", genomeset, "_", ss, "_natsel", ns, ".cfg"))
      #出力先フォルダ作成
      dir.create(paste0("/Users/katsu/Dropbox/5_Simulation/Result/", genomeset, "_", ss, "_natsel", ns))
    }}}
...
paste(
  ...
  cat(
    ., "\n", file = "./Config_discrete/config_marker.txt", 
...
paste(
  ...
  "markers.txt",     #marker file name
  "y",                 #Reconbination file
  4,                 #Reconbination N
  5,                 #Male female difference
  "recomb.txt",      #recombination file
  sep = "\n"
  ) %>% 
  cat(., "\n", file = "./Config_discrete/config_marker.txt", append = TRUE)
...
# Create gene file
gf <- make_genome()
write_tsv(gf, "./Config_learn/genes.txt")
...
write_tsv(Pheno, "./Config_learn/phenotypes_learn.txt")  

# Config file
for(ss in c("obl_posit", "obl_negat")){
  for(ns in c(0.3, 1)){
    make_config(gset = "learn", sexsel = ss, natsel = ns) %>% 
      write(paste0("./Config_learn/learn_", ss, "_natsel", ns, ".cfg"))
    #出力先フォルダ作成
...
paste(
  ...
  cat(
    ., "\n", file = "./Config_learn/config_marker.txt", 
    append = FALSE  #Renew file
    )
...
paste(
  ...
  cat(., "\n", file = "./Config_learn/config_marker.txt", append = TRUE)
...
# Genes file
gf <- make_genome()
write_tsv(gf, "./Config_quant/genes.txt")
...
write_tsv(Pheno, "./Config_quant/phenotypes_quant.txt")  

# Config file
for(ss in c("constchoosy", "hybridrandom")){
  for(ns in c(0.3, 1)){
    make_config(gset = "quant", sexsel = ss, natsel = ns) %>%
      write(paste0("./Config_quant/quant_", ss, "_natsel", ns, ".cfg"))
    #出力先フォルダ作成
...
paste(
  ...
  cat(
    ., "\n", file = "./Config_quant/config_marker.txt", 
    append = FALSE  #Renew file
    )
...
paste(
  ...
  cat(., "\n", file = "./Config_quant/config_marker.txt", append = TRUE)
