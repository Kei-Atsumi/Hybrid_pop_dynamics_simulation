...
make_bash <- function(gset = "", sexsel = "") {
  ...
  paste(
    ...
    "#PBS -q blade",
    "#PBS -l nodes=1:ppn=9,mem=90gb,walltime=48:00:00",
    "module load gcc",
    "REPS=101",
    "COUNTERA=0",
    "COUNTERB=0",
...
Pheno <- data.frame(
  Phenotypes = c("Sex", "BDMI", "Underdominance", "Pref0", "Signal0", "Hybridindex"), 
  Formula    = c(
    gf[1, "name"],   # Sex
...
# dis_link
Pheno[5,2] <- gf[2+g_chrom*13, "name"]
...
for(genomeset in c("dis_unlink", "dis_link")){
  for(ss in c("speriorhyb", "disruptive", "flip", "heterorandom", "pdominance", "recessive", "selfref")){
    ...
    for(BDMI in c(0.1, 0.4, 0.8)){
      for(H in c(0.1, 0.4, 0.8)){
